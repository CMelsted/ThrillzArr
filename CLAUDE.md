...
- **`importer/views.py`** — the whole user flow lives here as class-based views, wired in `importer/urls.py`. The flow is Import → Match → **Review** → (approve) → background processing:
  ...
  - `MatchView` (`/match`) — one entry is accepted (or removed) per POST: an Accept button per entry submits `src_path`+`asin`, validated against the configured API (`m4b_merge.helpers.validate_asin`), then `utils.merge.create_book` (creates the book in `Pending Review` status — nothing is enqueued yet) and the entry is removed from `request.session['input_dir']`; when the session list empties, redirects to `/review` (or `/` if nothing pending). A per-entry X posts `action=remove` to drop an entry from the session.
  ...
  - `ClearJobsView` (`/books/clear`) — bulk-deletes selected Done/Error/Abandoned books, or all Done books via `action=clear_all_done` (deletes the `Status`, which cascades to the `Book`; never touches files).
  - `EditBookView` (`/book/<id>/edit`) — post-completion metadata editing, reuses `BookMetadataForm`.
  - `PresetsView` (`/presets`) — CRUD for `ConversionPreset` (output path scheme, move-to-Audiobookshelf path; exactly one marked default, enforced in `ConversionPreset.save()`).
  - `SettingView` (`/setting`) — single-row `Setting` CRUD (create-if-none-else-update, never more than one row): `api_url`, `num_cpus`, `delete_source_after_success`, and an optional completed-directory (blank = moving disabled; the form exposes it behind a `move_completed_enabled` checkbox). Input/output paths are NOT settings — they come from the deploy-time bind mounts (`/input`, `/output`, with `~/input`/`~/output` fallbacks outside Docker; see `rootdir` in `importer/views.py` and `output_dir()` in `utils/merge.py`). The settings page also hosts the client-side light/dark toggle.
