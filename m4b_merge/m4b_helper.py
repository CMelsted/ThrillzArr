...
class M4bMerge:
    ...
    def _run_subprocess(self, args):
        # Run the child in its own session/process group so the whole
        # tree (m4b-tool and any ffmpeg workers it spawns) can be killed
        # with os.killpg() if the job is abandoned.
        process = subprocess.Popen(args, shell=False, start_new_session=True)
        self.last_pgid = os.getpgid(process.pid)
        if self.on_subprocess_start:
            self.on_subprocess_start(self.last_pgid)
        try:
            return process.wait()
        finally:
            self.last_pgid = None
    ...
    def run_merge(self):
        # Compatibility wrapper running all stages in order.
        # Callers wanting per-stage progress/cancellation should invoke
        # the stages individually instead.
        self.prepare_data()
    ...
    def merge(self):
        ...
        # Requires prepare_data() and prepare_command_args() to have run.
